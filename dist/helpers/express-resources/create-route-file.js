...
const createRouteFile = (routeId, method, extension) => {
    const routeFileName = `${(0, lodash_1.camelCase)((0, get_component_filename_1.getComponentFileName)(routeId, "", method))}Route`;
...
`;
    const filename = extension
        ? (0, get_component_filename_1.getComponentFileName)(routeId, extension, method)
        : (0, get_component_filename_1.getComponentFileName)(routeId, ".ts", method);
    const directoryPath = `${(0, get_component_root_1.getComponentRoot)(`${routeId}/${routeId}`)}`;
    const installDirectory = `${(0, process_1.cwd)()}/src/routes/${directoryPath}`;
